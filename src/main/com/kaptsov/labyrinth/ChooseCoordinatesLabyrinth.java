...
public class ChooseCoordinatesLabyrinth {
    public ChooseCoordinatesLabyrinth(int numberLabyrinth, JFrame jFrame, DatabaseLabyrinths databaseLabyrinths) {
        Labyrinths labyrinths = databaseLabyrinths.getLabyrinths(numberLabyrinth);
        JPanel jPanel1 = new JPanel();
        int x = labyrinths.getData()[0].length;
        int y = labyrinths.getData().length;
        JPanel jPanel[] = new JPanel[y];
        JButton jButtons[][] = new JButton[y][x];
        ...
        for (int i = 0; i < y; i++) {
            ...
            for (int j = 0; j < x; j++) {
                ...
                jButton.setSize(3, 6);
                if (!labyrinths.getData()[i][j]) jButton.setBackground(Color.darkGray);
                else {
                    int finalX = j+1;
                    int finalY = i+1;
                    jButton.addActionListener(e -> {
                        DataToStart dataToStart = new DataToStart(finalX, finalY, labyrinths);
